...
class ModelAccountCustomerSupport extends Model {
	...
	public function addCustomerSupport($data = array()){
		...
		$subject = "[".$this->config->get('config_title')."] ".html_entity_decode($category_data['customer_support_1st_category'].'/'.$category_data['customer_support_2nd_category'].'/'.$data['subject'], ENT_QUOTES, 'UTF-8');
		// HTML Mail
		$template = new Template();
		
		$template->data['title'] 			= $language->get('text_subject');
		$template->data['text_greeting'] 	= sprintf($language->get('text_greeting'), html_entity_decode($this->config->get('config_title'), ENT_QUOTES, 'UTF-8'));
		...
		$template->data['store_name'] 		= $this->config->get('config_title');
		...
		// Text Mail
		$text  = sprintf($language->get('text_greeting'), html_entity_decode($this->config->get('config_title'), ENT_QUOTES, 'UTF-8')) . "\n\n";
