...
class ModelSaleOrder extends Model {
	...
	public function getOrder($order_id) {
		$order_query = $this->db->query("SELECT *, (SELECT GROUP_CONCAT( CONCAT(op.quantity,  ' x ', op.name) SEPARATOR '\n' ) FROM `order_product` op
						WHERE op.order_id = o.order_id) as products,  (SELECT CONCAT(c.firstname, ' ', c.lastname) FROM " . DB_PREFIX . "customer c WHERE c.customer_id = o.customer_id) AS customer FROM `" . DB_PREFIX . "order` o WHERE o.order_id = '" . (int)$order_id . "'");
		...
		if ($order_query->num_rows) {
			...
			return array(
				...
				'reward'                  => $reward,
'products'                => $order_query->row['products'],
	...
	public function getOrders($data = array()) {
		$sql = "SELECT o.order_id, o.comment, CONCAT(o.firstname, ' ', o.lastname, ' / ', o.shipping_country) AS customer, (SELECT os.name FROM " . DB_PREFIX . "order_status os WHERE os.order_status_id = o.order_status_id AND os.language_id = '" . (int)$this->config->get('config_language_id') . "') AS status, o.total, o.currency_code, o.currency_value, o.date_added, o.date_modified, o.shipping_method, o.payment_method, (SELECT GROUP_CONCAT( CONCAT(op.quantity,  ' x ', op.model) SEPARATOR '<br />' ) AS p FROM `order_product` op
		...
		if (!empty($data['filter_customer'])) {
			$sql .= " AND LCASE(CONCAT(o.firstname, ' ', o.lastname, ' / ', o.shipping_country)) LIKE '%" . $this->db->escape(utf8_strtolower($data['filter_customer'])) . "%'";
		}
	...
	public function addOrderHistory($order_id, $data) {
		...
		$this->db->query("INSERT INTO " . DB_PREFIX . "order_history SET order_id = '" . (int)$order_id . "', user='". $this->user->getUserName() . "', order_status_id = '" . (int)$data['order_status_id'] . "', notify = '" . (isset($data['notify']) ? (int)$data['notify'] : 0) . "', comment = '" . $this->db->escape(strip_tags($data['comment'])) . "', date_added = NOW()");
		...
		if ($this->config->get('config_complete_status_id') == $data['order_status_id']) {
      	...
      	if ($data['notify']) {
			...
			$message = "---------------------------------------------\n";
	$message  .= sprintf("%-20s&nbsp;&nbsp;&nbsp;%s\n", $language->get('text_order'), $order_id);
	
			$message  .= sprintf("%-20s %s\n", $language->get('text_date_added'), date($language->get('date_format_short'), strtotime($order_info['date_added'])));
			
			$order_status_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_status WHERE order_status_id = '" . (int)$data['order_status_id'] . "' AND language_id = '" . (int)$order_info['language_id'] . "'");
				

	$message  .= sprintf("%-20s&nbsp;%d %s\n", $language->get('text_order_total'), $order_info['total'], $order_info['currency_code']);
	
			if ($order_status_query->num_rows) {
				
				
	$message  .= sprintf("%-20s %s\n", $language->get('text_order_status'), $order_status_query->row['name']);
	
			}
			

	$message  .= sprintf("---------------------------------------------\n");
	$message  .= sprintf("%s\n%s\n\n", $language->get('text_order_products'), $order_info['products']);
...
$message .= "=============================================\n\n";
	...
	public function getOrderHistories($order_id, $start = 0, $limit = 10) {
		...
		$query = $this->db->query("SELECT oh.date_added, oh.order_history_id, oh.user,  os.name AS status, oh.comment, oh.notify FROM " . DB_PREFIX . "order_history oh LEFT JOIN " . DB_PREFIX . "order_status os ON oh.order_status_id = os.order_status_id WHERE oh.order_id = '" . (int)$order_id . "' AND os.language_id = '" . (int)$this->config->get('config_language_id') . "' ORDER BY oh.date_added, oh.order_history_id ASC LIMIT " . (int)$start . "," . (int)$limit);
