...
class ControllerModuleNotifyWhenArrives extends Controller {
	...
	$this->document->setTitle($this->language->get('heading_title'));
        ...
        $this->document->addScript('view/javascript/nwa/jquery.tablesorter.min.js');
        $this->document->addScript('view/javascript/nwa/filter.js');
	$this->document->addStyle('view/stylesheet/nwa.css');
	...
	$this->data['entry_nwa_use_captcha'] = $this->language->get('entry_nwa_use_captcha');
        
	$this->data['entry_nwa_image_size'] = $this->language->get('entry_nwa_image_size');
 
	$this->data['entry_filter'] = $this->language->get('entry_filter');
	$this->data['text_filter_last_year'] = $this->language->get('text_filter_last_year');
	$this->data['text_filter_last_6_months'] = $this->language->get('text_filter_last_6_months');
	$this->data['text_filter_last_3_months'] = $this->language->get('text_filter_last_3_months');
	$this->data['text_filter_last_month'] = $this->language->get('text_filter_last_month');
	$this->data['text_filter_last_week'] = $this->language->get('text_filter_last_week');
	$this->data['text_filter_all'] = $this->language->get('text_filter_all');
	$this->data['text_filter_custom'] = $this->language->get('text_filter_custom');
	
	$this->data['text_filter_date_start'] = $this->language->get('text_filter_date_start');
	$this->data['text_filter_date_end'] = $this->language->get('text_filter_date_end');
	$this->data['text_filter_graphic_max'] = $this->language->get('text_filter_graphic_max');
        
	$this->data['text_heading_system'] = $this->language->get('text_heading_system');
	$this->data['text_heading_display'] = $this->language->get('text_heading_display');
	$this->data['text_heading_install'] = $this->language->get('text_heading_install');
        
	$this->data['button_update_statistics'] = $this->language->get('button_update_statistics');
	$this->data['button_multi_delete'] = $this->language->get('button_multi_delete');
	
	$this->data['text_confirm_multi_delete'] = $this->language->get('text_confirm_multi_delete');
    ...
    if (isset($this->request->post['nwa_use_name'])) {
	 ...
	 if (isset($this->request->post['nwa_use_captcha'])) {
	    $this->data['nwa_use_captcha'] = $this->request->post['nwa_use_captcha'];
	} else {
	    $this->data['nwa_use_captcha'] = $this->config->get('nwa_use_captcha');
	}  
   	 if (isset($this->request->post['nwa_image_width'])) {
	    $this->data['nwa_image_width'] = $this->request->post['nwa_image_width'];
	} else {
	    $this->data['nwa_image_width'] = $this->config->get('nwa_image_width');
	} 
        
       if (isset($this->request->post['nwa_image_height'])) {
	    $this->data['nwa_image_height'] = $this->request->post['nwa_image_height'];
	} else {
	    $this->data['nwa_image_height'] = $this->config->get('nwa_image_height');
	}   
			...
			$mail_admin_message = html_entity_decode($this->config->get('notify_when_arrives_mail_'.$language_id.'_admin'), ENT_QUOTES, 'UTF-8');
			    ...
			    $mails = $request['mails'];
                            ...
                            if (!$keyword || !$this->config->get('config_seo_url')){
                                  ...
                                  $link_href =  $stores[$store_id]['url'] . 'index.php?route=product/product&amp;product_id=' . $id;								  $link_short = "product_id=" . $id;
                                
                            }else{
                                
                                
                                  $link = '<a href="' . $stores[$store_id]['url'] . $keyword.'">'. $stores[$store_id]['url'] . $keyword. '</a>';
                                  $link_href = $stores[$store_id]['url'] . $keyword;								  $link_short = $keyword;
                                         
                            }
                            
                            $store_name = $stores[$store_id]['name'];

			    $subject = str_replace(array('{store_name}', '{product_name}', '{product_link}','{product_image}', '{product_href}','{product_image_href}'), array($store_name, $name, $link, $image,$link_href,$image_href), $mail_subject);
			    $message = str_replace(array('{store_name}', '{product_name}', '{product_link}','{product_image}', '{product_href}','{product_image_href}'), array($store_name, $name, $link, $image,$link_href,$image_href), $mail_message);
	...
	$date_start = isset($this->request->get['date_start']) ? $this->request->get['date_start'] : null;
	$date_end = isset($this->request->get['date_end']) ? $this->request->get['date_end'] : null;
	
	
	  $this->response->setOutput(json_encode($this->model_module_notify_when_arrives->getList(false,$date_start,$date_end)));
