...
class ControllerModuleNotifyWhenArrives extends Controller {
    ...
    public function index() {
        ...
        if ($this->config->get('notify_when_arrives_status')) {
            ...
            $this->document->addScript('catalog/view/javascript/nwa.min.js');

            if (file_exists('catalog/view/theme/' . $this->config->get('config_template') . '/stylesheet/nwa.css')) {
                    $this->document->addStyle('catalog/view/theme/' . $this->config->get('config_template') . '/stylesheet/nwa.css');
            } else {
                    $this->document->addStyle('catalog/view/theme/default/stylesheet/nwa.css');
            }
	    ...
	    $this->data['nwa_entry_captcha'] = $this->language->get('nwa_entry_captcha');
    ...
    public function register() {
        ...
        if (isset($this->request->post['notify_email']) && isset($this->request->post['notify_product_id']) && $this->validate()) {
    ...
    private function validate() {
	 ...
	 $captcha = isset($this->request->post['nwa_captcha']) ? $this->request->post['nwa_captcha'] : '';

        if ($this->config->get('nwa_use_captcha') && (empty($this->session->data['nwa_captcha']) || ($this->session->data['nwa_captcha'] != $captcha))) {

            $this->data['text_error'] = $this->language->get('text_error_captcha');
        }
    ...
    public function captcha() {
	    ...
	    $this->session->data['nwa_captcha'] = $captcha->getCode();
